...
pub const ALDRIN_V1_SWAP_DISCRIMINATOR: [u8; 8] = [248, 198, 158, 145, 225, 117, 135, 200];
pub const ALDRIN_V1_MIN_ACCOUNTS: usize = 7;

#[derive(Clone)]
pub struct AldrinV1Accounts<'info> {
...
pub fn aldrin_v1_swap<'info>(
    accounts: AldrinV1Accounts<'info>,
    amount_in: u64,
    ...
    let ix = Instruction {
        ...
        accounts: metas,
        data: aldrin_v1_swap_data(amount_in, minimum_amount_out, base_to_quote),
    };
...
fn aldrin_v1_swap_data(amount_in: u64, minimum_amount_out: u64, base_to_quote: bool) -> Vec<u8> {
    let mut data = Vec::with_capacity(25);
    data.extend_from_slice(&ALDRIN_V1_SWAP_DISCRIMINATOR);
...
mod tests {
    ...
    fn instruction_data_matches_official_sdk_layout() {
        let bid = aldrin_v1_swap_data(1_604, 20_000, false);
        assert_eq!(bid.len(), 25);
        assert_eq!(&bid[..8], &ALDRIN_V1_SWAP_DISCRIMINATOR);
        assert_eq!(&bid[8..16], &1_604_u64.to_le_bytes());
        assert_eq!(&bid[16..24], &20_000_u64.to_le_bytes());
        assert_eq!(bid[24], 0);
    }
}
