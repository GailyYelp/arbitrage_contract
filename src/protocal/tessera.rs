...
pub const TESSERA_MIN_ACCOUNTS: usize = 10;
pub const TESSERA_SWAP_SELECTOR: u8 = 0x10;

fn side_for_direction(direction: u8) -> Option<u8> {
    match direction {
        0 => Some(1),
        1 => Some(0),
        _ => None,
...
pub fn tessera_swap<'info>(
    accounts: TesseraAccounts<'info>,
    amount_in: u64,
    min_amount_out: u64,
    direction: u8,
) -> Result<SwapResult> {
    let side = side_for_direction(direction)
        .ok_or(crate::errors::ArbitrageError::InvalidInstructionData)?;
    ...
    data.push(TESSERA_SWAP_SELECTOR);
    data.push(side);
...
mod tests {
    ...
    #[test]
    fn direction_maps_to_tessera_side() {
        assert_eq!(side_for_direction(0), Some(1));
        assert_eq!(side_for_direction(1), Some(0));
        assert_eq!(side_for_direction(2), None);
    }
}
